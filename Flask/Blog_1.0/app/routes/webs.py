...
def index():
    ...
    users = [User.query.get(post.user_id) for post in posts]

    return render_template("index.html", title='Home Page', pairs=zip(users, posts), form=form)
...
def user(username):
    ...
    posts_: List[Post] = Post.get_posts(user_.id)
